...
import { CtxFormSelector, CtxAfterSuccessfulSubmit, CtxErrorsInForm } from "../Ctxs";
...
export function BaseForm({apiItem,children,data,disableSubmit = false,...props}:BaseFormAttrs){
    ...
    let [errosState,setErrors] = useState({} as FieldErrors<any>);
    ...
    function onInvalid(e:FieldErrors<any>){
        setErrors(e);
    }
    return (
        <CtxErrorsInForm.Provider value={errosState}>
            <form {...props} hidden={noHidden != name} onSubmit={handleSubmit(onValid,onInvalid)} >
